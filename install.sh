...
# GitHub CLI
run_step 4 "Installing GitHub CLI" bash -c '
  GH_DEB=$(mktemp)
...
# MongoDB
run_step 7 "Installing MongoDB" bash -c '
...
# AWS CLI
run_step 8 "Installing AWS CLI" bash -c '
...
# EB CLI in venv
run_step 9 "Installing AWS EB CLI" bash -c '
...
# fonttools in venv
run_step 10 "Installing fonttools in venv" bash -c '
...
# Glyphhanger
run_step 12 "Installing Glyphhanger via npm" bash -c '
  sudo apt-get install -y nodejs npm
  sudo npm install -g glyphhanger
'

# NVM
run_step 13 "Installing NVM" bash -c '
...
# GNOME scaling
run_step 14 "Setting GNOME scaling factor" bash -c '
  gsettings set org.gnome.desktop.interface scaling-factor 0.88 || true
'

# Google Chrome
run_step 15 "Installing Google Chrome" bash -c '
  CHROME_DEB=$(mktemp)
...
run_step 17 "Installing ImageMagick AppImage" bash -c '
  mkdir -p ~/AppImages
...
# Slack (deb)
run_step 19 "Installing Slack" bash -c '
  SLACK_DEB=$(mktemp)
